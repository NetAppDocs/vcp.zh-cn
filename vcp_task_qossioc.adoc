...
NOTE: 对于 NetApp HCI 部署，默认密码在安装过程中随机生成。要确定密码，请参阅本文中的步骤 4 https://kb.netapp.com/on-prem/solidfire/Element_OS_Kbs/mNode_Status_Network_Down_or_Down_in_the_mNode_Settings_tab_of_Element_Plugin_for_vCenter_VCP["知识库"^]。
...
NOTE: 请参阅此处 https://kb.netapp.com/on-prem/solidfire/Element_OS_Kbs/mNode_Status_Network_Down_or_Down_in_the_mNode_Settings_tab_of_Element_Plugin_for_vCenter_VCP["知识库"^]进行故障排除（如果状态为以下任何内容）：* `Down`：未启用 QoSSIOC。* `Not Configured`：未配置 QoSSIOC 设置。* `Network Down`：vCenter 无法与网络上的 QoSSIOC 服务通信。mNode 和 SIOC 服务可能仍在运行。
