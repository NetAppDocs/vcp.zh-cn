...
[role="lead"]
您可以为插件控制的单个卷和数据存储库设置 QoSSIOC 自动化。 link:vcp_concept_qossioc.html["QoSSIOC"] 是自动服务质量 (https://docs.netapp.com/us-en/hci/docs/concept_hci_performance.html["QoS"^]）基于存储 I/O 控制 ）标准数据存储库上所有 VM 的设置。
